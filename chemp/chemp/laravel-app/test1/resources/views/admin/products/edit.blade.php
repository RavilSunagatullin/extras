...
@section('content')
     ...
     <a href="{{route("admin.index")}}" class="border bg px-10 fz-32 text-center">Admin panel</a>
                ...
                <form action="{{route('admin.product.update', $product->id)}}" method="post" class="mw-500 mx-auto" enctype="multipart/form-data">
                @csrf
                @method('patch')
                ...
                <div class="column px-10 border">  
                    <img src="{{asset('storage/'.$product->image)}}" alt="{{$product->title}} image">
